...
class AutomatedTool(QMainWindow):
    ...
    def create_about(self):
        ...
        about_label.setText("""
            ...
            <p>Com apenas <strong>21 anos</strong>, ele se destaca como um verdadeiro estrategista no mundo da <strong>cibersegurança</strong>.</p>
            <p>Para ele, não basta apenas encontrar falhas. Criar <strong>ferramentas automatizadas</strong> não é apenas um hábito — é uma arte.</p>
    ...
    def start_scan(self):
        ...
        if not domain:
            QMessageBox.warning(self, "Campo vazio", "Por favor, insira um domínio ou subdomínio para escanear.")
            return

        self.progress_bar.setValue(0)
        try:
            xss_tool.scan_xss(domain)
            ...
            QMessageBox.critical(self, "Erro durante o scan", f"Ocorreu um erro: {e}")

if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = AutomatedTool()
    window.set_background_image("/home/kali/Ferramentas/sentinela_primeiro/assets/logo.ico")
    window.show()
    sys.exit(app.exec())
